/**
 * Death Claims Account Holder List Page JavaScript
 */
...
function loadApplications() {
    ...
    fetch(`${BASE_PATH}api/get_ac_holder_death_claims_applications.php`)
...
function applyFilters() {
    ...
    filteredData = allData.filter(app => {
        ...
        const matchSearch = !search || 
            app.deceased_name.toLowerCase().includes(search) ||
...
function displayTable() {
    ...
    tbody.innerHTML = pageData.map((app, idx) => {
        ...
        const deathDate = app.death_date ? new Date(app.death_date).toLocaleDateString('hi-IN') : 'N/A';
        
        return `
            <tr>
                <td>${serialNo}</td>
                <td>${escapeHtml(app.account_holder_name || app.deceased_name || 'N/A')}</td>
                <td>50</td>
                <td>${escapeHtml(app.district || 'Unknown')}</td>
                <td>${escapeHtml(app.block || 'Unknown')}</td>
                <td>${deathDate}</td>
                <td>
                    <a href="applicante-death-claims-sahyog-suchi.php?member_id=${app.member_id}" class="btn btn-sm btn-primary">
                        View Details
                    </a>
                </td>
            </tr>
